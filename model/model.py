...
class GeneratorWithSparseVector(nn.Module):
    def __init__(self, scale=2):
        super(GeneratorWithSparseVector, self).__init__()
        self.sparse = sparsity.SparseBlock() #channel=32
        ...
        self.sft1 = sparsity.SFTLayer(in_feat=32, out_feat=64)
        ...
        self.sft2 = sparsity.SFTLayer(in_feat=32, out_feat=64+32)
    ...
    def forward(self, noise, lr):
        ...
        up1 = self.sft1(up1, F.interpolate(sparse, scale_factor=1/4))

        up2 = self.up2(up1)
        up2 = self.up_conv2(torch.cat([up2, d2], dim=1))
        up2 = self.sft2(up2, F.interpolate(sparse, scale_factor=1/2))

        up3 = self.up3(up2)
        up3 = self.up_conv3(torch.cat([up3, d1], dim=1))
        up3 = self.sft3(up3, sparse)
...
class LinearDownsampler(nn.Module):
    def __init__(self, scale):
        super(LinearDownsampler, self).__init__()
        self.downsampler = nn.Sequential(
            nn.Conv2d(3, 32, kernel_size=3, stride=1, padding=5, dilation=5, bias=False),
            nn.Conv2d(32, 32, kernel_size=3, stride=1, padding=1, bias=False),
            nn.Conv2d(32, 32, kernel_size=3, stride=1, padding=1, bias=False),
            PixelUnShuffle(scale),
            nn.Conv2d(32*scale*scale, 32, kernel_size=1, stride=1, padding=0, bias=False),
